module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    if (!ADMINBOT.includes(senderID) && adminBot.adminOnly == true) {
      if (!ADMINBOT.includes(senderID) && adminBot.adminOnly == true) return api.sendMessage('[🐧] MIRAI [🐧] - Chỉ Admin Bot mới có thể sử dụng bot', threadID, messageID)
    }
    if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && !NDH.includes(senderID) && ndhBot.ndhOnly == true) {
      if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID)  && !NDH.includes(senderID) && ndhBot.ndhOnly == true) return api.sendMessage('[🐧] MIRAI [🐧] - Chỉ người hỗ trợ bot mới có thể sử dụng bot', threadID, messageID)
    }
    
    const dataAdbox = require('./../../modules/commands/cache/data.json');
    var threadInf = (threadInfo.get(threadID) || await Threads.getInfo(threadID));
    const findd = threadInf.adminIDs.find(el => el.id == senderID);
    if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('[🐧] MODE [🐧] - Chỉ Admin Box mới được sử dụng bot!!', event.threadID, event.messageID)
